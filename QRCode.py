...
custom = st.number_input("Type the number of the corresponding S-N curves you want to display, or type 0 for a customized comparison:", 
...
if 1 <= custom <= len(curve_comp):
    ...
    T_input = st.number_input("Enter the plate thickness (mm):", min_value=0, step=1)
    ...
    custom_list = st.text_input("Type the numbers corresponding to the S-N curves you want to display, separated by a space:")
    if st.button("Generate Custom Curves"):
        try:
            ...
            for curve in custom_curves:
                T_input = st.number_input(f"Enter the plate thickness (mm) for {curve.name}:", min_value=1, step=1, key=curve.name)
                Nf_vals, S_vals = curve.generate_points(T_input)
                ax.loglog(Nf_vals, S_vals, label=f"{curve.name} (T={int(T_input)} mm)")
            ax.set_xlabel("Fatigue Life (Cycles)")
            ax.set_ylabel("Stress Range (MPa)")
